...
## 🧩 Day 2 Homework Requirements Met

| Requirement | Status |
|-------------|--------|
| 3+ stages each using a named technique | ✅ UNDERSTAND (role + structured output), REASON (chain-of-thought), PRODUCE (goal-oriented + constraints) |
| Structured JSON handoff between stages | ✅ Every stage consumes/produces JSON |
| At least one chain-of-thought stage | ✅ Stage 2 (REASON) |
| One broken input handled gracefully | ✅ Gibberish test → Unknown customer, other category, neutral sentiment |
| Full run with every stage visible | ✅ CLI prints all stages; web UI shows input/raw/parsed for each |
| One-paragraph reflection on weakest link | ✅ Stage 1 identified with Day 4/6 upgrade path |
